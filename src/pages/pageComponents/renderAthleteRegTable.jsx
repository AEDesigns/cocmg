...
const AthletesList = () => {
  ...
  const fetchDataUrl = "http://localhost:3000/athletics";
  // useEffect hook to fetch data when the component mounts
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(fetchDataUrl); // Fetch athletes data from API
        if (!response.ok) {
  ...
  }, [fetchDataUrl]); // Empty dependency array means this runs only once when the component mounts
    ...
    <div>
      <Container>
        <Row>
          ...
          <Table>
            ...
            <tbody>
              <tr>
                ...
                {[...new Set(athletes.map((athlete) => athlete.class))].map(
                  (className) => (
                    <td key={className}>
                      {athletes
                        .filter((athlete) => athlete.class === className)
          ...
          <small>
            *Registration fees due when you check in for the games on game day*
          </small>
        </Row>
      </Container>
    </div>
